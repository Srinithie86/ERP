...
class LeadService {
  ...
  // static Future<List<dynamic>> fetchFollowUpHistoryAll({String? leadId}) async {
  //   try {
  //     final prefs = await SharedPreferences.getInstance();
  //     String deviceId = prefs.getString('device_id') ?? '';
  //     String ln = prefs.getString('ln') ?? '';
  ...
  //       'type': '3016',
  //       'cid': currentCid,
  //       'led_id': leadId ?? '', // Use explicit leadId for 3016
  //       'uid': uid ?? '',
  //       'lt': lt,
  //       'ln': ln,
  //       'device_id': deviceId,
  ...
  //     debugPrint(">>> FETCH HISTORY ALL (3016) REQUEST: leadId=$leadId");
  //     debugPrint(">>> BODY: $body");
  ...
  //         var items = data['data'] ?? data['details'];
  ...
  static Future<List<dynamic>> fetchCallSummary(String leCode) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      String deviceId = prefs.getString('device_id') ?? '';
      String ln = prefs.getString('ln') ?? '';
      ...
      final Map<String, String> body = {
        ...
        'select': '*',
        'where': 'le_code=$leCode', 
        'role_id': await PreferenceService.getRoleId(),
        if (token != null) 'token': token,
      };

      debugPrint(">>> CALL SUMMARY REQUEST Body: $body");

      final response = await http.post(Uri.parse(_apiUrl), body: body);

      debugPrint(">>> CALL SUMMARY RESPONSE Status: ${response.statusCode}");
      debugPrint(">>> CALL SUMMARY RESPONSE Body: ${response.body}");

      if (response.statusCode == 200) {
        final Map<String, dynamic> data = _safeDecodeJson(response.body);

        if (data['error'] == false || data['error'].toString() == 'false' || data['status'] == 'success') {
          var listData = data['data'] ?? data['details'];
          if (listData is List) {
            return List<dynamic>.from(listData);
          }
  ...
  static Future<List<dynamic>> fetchDropdownData({
    ...
    try {
      ...
      debugPrint(
        "------------ FETCH DROPDOWN DATA REQUEST (Type: $type, ListId: $listId) ------------",
      );
      debugPrint("URL: $_apiUrl");
      debugPrint("BODY: $body");
      ...
      if (response.statusCode == 200) {
        ...
        if (data['error'] == false) {
          var listData = data['details'] ?? data['data'] ?? data['dropdown'];
