...
class _AdminApprovalsScreenState extends State<AdminApprovalsScreen> {
  ...
  Widget _buildBody() {
    switch (_selectedFilter) {
      case "All":
        return const _InterleavedApprovalsFeed();
      case "Leave":
        return const AdminLeaveRequestsScreen(showAppBar: false);
      case "Permission":
        return const AdminPermissionRequestsScreen(showAppBar: false);
      case "Expense":
...
class _InterleavedApprovalsFeedState extends State<_InterleavedApprovalsFeed> {
  ...
  Future<void> _fetchAndCombine() async {
    ...
    try {
      ...
      leaveFuture.then((resp) => _processBatch("Leave", resp.data));
      permFuture.then((resp) => _processBatch("Permission", resp.data));
      expFuture.then((resp) {
        final List expData = resp['data'] ?? [];
        _processBatch("Expense", expData);
      });
  ...
  void _processBatch(String type, List<dynamic> data) {
    if (!mounted) return;

    final List<dynamic> newItems = [];
    if (type == "Expense") {
      newItems.addAll(data.where((doc) {
      ...
      newItems.addAll(data.where((doc) {
        ...
        return s == "pending" || s == "" || s == "0";
      }).map((e) => {"type": type, "data": e, "id": e.id}));
    }
