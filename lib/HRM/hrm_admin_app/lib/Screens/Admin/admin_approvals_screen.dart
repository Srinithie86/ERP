...
class _AdminApprovalsScreenState extends State<AdminApprovalsScreen> {
  String _selectedFilter = "All"; // 'All', 'Leave', 'Permission'
  ...
  Widget _buildBody() {
    if (_selectedFilter == "Leave") {
      ...
      // "All" - Interleaved feed of both Leave and Permission requests
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (item['type'] == "Leave") {
            // We use a simplified version of the card or the actual screen widget with single item
            // For now, to keep it clean, we'll use a text label then the card (or just the card if it's clear)
            return _buildTypeHeader(context, "Leave Request", item['data']);
          } else {
            return _buildTypeHeader(context, "Permission Request", item['data']);
          }
        },
      ),
    );
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: title.contains("Leave") ? Colors.blue : Colors.orange,
