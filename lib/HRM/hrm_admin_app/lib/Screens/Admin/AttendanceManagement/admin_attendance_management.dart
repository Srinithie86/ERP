...
class AdminAttendanceManagementScreen extends StatefulWidget {
  ...
  @override
  State<AdminAttendanceManagementScreen> createState() => _AdminAttendanceManagementScreenState();
}

class _AdminAttendanceManagementScreenState extends State<AdminAttendanceManagementScreen> with SingleTickerProviderStateMixin {
  ...
  Future<List<AttendanceData>> _fetchAllAttendance() async {
    try {
      final results = await Future.wait([
        AttendanceApi.fetchMobileAttendance(),
        AttendanceApi.fetchInOfficeAttendance(),
        AttendanceApi.fetchBreakAttendance(),
        AttendanceApi.fetchMarketingAttendance(),
      ]);
        ...
        bottom: PreferredSize(
          ...
          child: Container(
            ...
            child: TabBar(
              ...
              labelStyle: GoogleFonts.outfit(fontSize: 13.sp, fontWeight: FontWeight.w700),
              unselectedLabelStyle: GoogleFonts.outfit(fontSize: 13.sp, fontWeight: FontWeight.w500),
              tabs: const [
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            bool isMobile = record.loc != null || record.selfie != null || record.screenshot != null;
            return isMobile ? _buildMobileCard(record) : _buildOfficeCard(record);
          },
        );
      },
    );
  ...
  Widget _buildOfficeTab() {
    return FutureBuilder<AttendanceResponse>(
      ...
      builder: (context, snapshot) {
        ...
        if (records.isEmpty) return const Center(child: Text("No valid records found"));
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Container(
                        ...
                        child: (record.selfie == null && record.screenshot == null)
                            ? Icon(Icons.person_rounded, size: 30.sp, color: const Color(0xFF94A3B8))
                            : null,
                      ),
                      SizedBox(width: 16.w),
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Icon(Icons.location_on_rounded, size: 12.sp, color: const Color(0xFF26A69A)),
                                SizedBox(width: 6.w),
                                Expanded(
                                  child: Text(
                                    record.loc ?? "Location N/A", 
                                    style: GoogleFonts.poppins(fontSize: 11.sp, color: const Color(0xFF64748B), fontWeight: FontWeight.w500), 
                                    maxLines: 1, 
                  ...
                  Row(
                    ...
                    children: [
                      _buildTimeInfo("Check In", record.inTime ?? "--:--", const Color(0xFF26A69A)),
                      _buildDetailedInfo("Date", record.date ?? "--/--/--", const Color(0xFF64748B)),
                      _buildTimeInfo("Check Out", record.outTime ?? "--:--", const Color(0xFFF59E0B)),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildTimeInfo(String label, String value, Color color) {
    return Column(
      ...
      children: [
        Text(
          label.toUpperCase(), 
          style: GoogleFonts.poppins(fontSize: 9.sp, color: const Color(0xFF94A3B8), fontWeight: FontWeight.w700, letterSpacing: 0.5),
        ),
        SizedBox(height: 4.h),
        Text(
          value, 
          style: GoogleFonts.outfit(fontSize: 15.sp, fontWeight: FontWeight.w700, color: color),
        ),
      ],
    );
  }
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(16.r),
                          border: Border.all(color: const Color(0xFF26A69A).withOpacity(0.1), width: 1.5),
                        ),
                        child: Center(
                          child: Text(
                  ...
                  if (record.clientName?.isNotEmpty == true || record.remarks?.isNotEmpty == true) ...[
                      ...
                      child: Column(
                        ...
                        children: [
                          if (record.clientName?.isNotEmpty == true)
                            Row(
                              children: [
                                Icon(Icons.business_rounded, size: 14.sp, color: const Color(0xFF26A69A)),
                                SizedBox(width: 8.w),
                                Expanded(
                                  child: Text(
                                    "CLIENT: ${record.clientName}",
                                    style: GoogleFonts.outfit(fontSize: 12.sp, fontWeight: FontWeight.w700, color: const Color(0xFF1E293B)),
                                  ),
                                ),
                              ],
                            ),
                          ...
                          if (record.remarks?.isNotEmpty == true)
                            Row(
                              children: [
                                Icon(Icons.info_outline_rounded, size: 14.sp, color: Colors.redAccent),
                                SizedBox(width: 8.w),
                                Expanded(
                                  child: Text(
                                    "NOTE: ${record.remarks}",
                                    style: GoogleFonts.poppins(fontSize: 11.sp, fontWeight: FontWeight.w600, color: const Color(0xFFEF4444)),
                                  ),
                                ),
                              ],
                            ),
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        _miniTimeColumn("TOTAL", record.totalHours ?? "0", const Color(0xFF3B82F6)),
                      ],
                    ),
                  ),
                ],
    ...
    return FutureBuilder<AttendanceResponse>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          padding: EdgeInsets.all(16.w),
          itemCount: records.length,
          itemBuilder: (context, index) => _buildOfficeCard(records[index]), // Reusing office card for break
        );
      },
    );
  }
    ...
    return FutureBuilder<AttendanceResponse>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          padding: EdgeInsets.all(16.w),
          itemCount: records.length,
          itemBuilder: (context, index) => _buildOfficeCard(records[index]), // Reusing office card for marketing
        );
      },
    );
  }
