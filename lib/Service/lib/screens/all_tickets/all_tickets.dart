...
class _AllTicketsScreenState extends State<AllTicketsScreen> {
  ...
  Future<void> _fetchTickets() async {
    ...
    try {
      ...
      if (res != null && res['error'] == false) {
        _tickets = res['records'] ?? [];
        _processAllTicketsData(); // Map and Sort once
        _applyFiltersAndGroup(); // Filter and Group
  ...
  void _processAllTicketsData() {
    _allProcessedTickets = _tickets.map((t) {
      ...
      final String rawStatus = (t['status']?.toString() ?? '').toUpperCase().trim();
      ...
      if (rawStatus.contains('ASSIGNED') || rawStatus == 'ACCEPTED') {
        ...
        filterCat = 'COMPLETED';
      } else if (rawStatus.contains('PENDING') || rawStatus.contains('OPENED')) {
        statusLabel = 'Opened';
        filterCat = 'OPENED';
      } else {
        // Fallback for safety
  ...
  String _mapStatusToFilter(String status) {
    ...
    if (s == 'ASSIGNED' || s == 'ACCEPTED' || s == 'IN PROGRESS') return 'ASSIGNED';
                            ...
                            children: [
                              ...
                              ...dayTickets.asMap().entries.map((entry) {
                                ...
                                return AllTicketsJobCard(
                                  ...
                                  dateText: t['dateText'],
                                  timeText: t['timeText'],
                                  label: t['status'], // Use the pre-computed statusLabel
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext sheetContext) {
        return _AssignBottomSheet(
          ...
          onAssign: (String priority, String engineerName, String engineerId) async {
            try {
...
class _AssignBottomSheet extends StatefulWidget {
  ...
  final void Function(String priority, String engineerName, String engineerId)
  onAssign;
