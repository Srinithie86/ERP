...
class _JobsScreenState extends State<JobsScreen> {
  ...
  Map<String, double> _jobCoordinatesForLocation(String location) {
    ...
    if (normalized.contains('floor 2')) {
      return {'lat': 11.0168, 'lng': 76.9558};
    }
    if (normalized.contains('admin')) {
      return {'lat': 11.0175, 'lng': 76.9564};
    }
    if (normalized.contains('meeting room')) {
      return {'lat': 11.0181, 'lng': 76.9549};
    }
      ...
      if (data['error'] == false && data['records'] != null) {
        ...
        setState(() {
          _apiJobs = records.map((record) {
            ...
            // Status Mapping
            final rawStatus = record['status']?.toString().toUpperCase() ?? '';
            String statusLabel;
            ...
            return _buildJob(
              ...
              customerName: (record['customer_id']?.toString().isEmpty ?? true)
                  ? 'N/A'
                  : record['customer_id'].toString(),
              ...
              complaint: (record['complaint_title']?.toString().isEmpty ?? true)
                  ? 'N/A'
                  : record['complaint_title'].toString(),
