...
class SalesInvoiceCatalogScreen extends StatefulWidget {
  ...
  const SalesInvoiceCatalogScreen({
    super.key,
    this.taxType = 'IGST',
  ...
  @override
  State<SalesInvoiceCatalogScreen> createState() => _SalesInvoiceCatalogScreenState();
}

class _SalesInvoiceCatalogScreenState extends State<SalesInvoiceCatalogScreen> {
  ...
  List<SalesInvoiceProduct> _products = [];
    ...
    try {
      ...
      if (res['error'] == false && res['data'] != null) {
        ...
        if (mounted) {
          setState(() {
            _products = data
                .map((item) => SalesInvoiceProduct(
                      id: item['id'].toString(),
                      name: item['product_name'] ?? 'Unknown',
                      stock: int.tryParse(item['stock_qty']?.toString() ?? '100') ?? 100,
  ...
  List<SalesInvoiceProduct> get _filteredProducts {
    List<SalesInvoiceProduct> result = _products;
  ...
  void _showDiscountBottomSheet(SalesInvoiceProduct product) {
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => SalesInvoicePaymentScreen(
  ...
  Widget _buildProductListItem(SalesInvoiceProduct product) {
    return Container(
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: product.isAdded ? Colors.redAccent : primaryColor,
