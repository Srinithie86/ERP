...
class SalesInvoicePaymentScreen extends StatefulWidget {
  ...
  const SalesInvoicePaymentScreen({
    super.key,
  ...
  @override
  State<SalesInvoicePaymentScreen> createState() => _SalesInvoicePaymentScreenState();
}

class _SalesInvoicePaymentScreenState extends State<SalesInvoicePaymentScreen> {
  ...
  Future<void> _submitInvoice() async {
    ...
    try {
      ...
      final Map<String, String> body = {
        ...
        'mobile': c?['phone']?.toString() ?? c?['Mobile']?.toString() ?? '',
      ...
      if (res['error'] == false) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(res['message'] ?? 'Failed to save invoice')),
          );
        }
      }
    } catch (e) {
  ...
  void initState() {
    ...
    _customerSearchController.text = _selectedCustomer?['Ledger_Name'] ?? _selectedCustomer?['label'] ?? '';
    _fetchCustomers();
    _fetchAllDropdowns();
  }
  ...
  void _fetchAllDropdowns() {
    _fetchDropdownData('17', _invoiceTypes, _invoiceTypeIds, (val) => _invoiceType = val, _invoiceType);
    _fetchDropdownData('16', _priceTypes, _priceTypeIds, (val) => _priceType = val, _priceType);
    _fetchDropdownData('26', _taxTypes, _taxTypeIds, (val) => _taxType = val, _taxType);
  }

  Future<void> _fetchDropdownData(String listId, List<String> targetList, Map<String, String> idMapping, Function(String) onSelectUpdate, String? currentSelection) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final cid = prefs.getString('cid') ?? '44555666';
      final lt = prefs.getString('lt') ?? '123';
        ...
        if (uniqueItems.isNotEmpty) {
          if (mounted) {
            setState(() {
              ...
              if (currentSelection == null || !targetList.contains(currentSelection)) {
  ...
  SalesInvoiceSummary get summary {
    double subtotal = 0;
    ...
    double shipping = 0.0;
    double tcs = 0.0; // Removed tcs logic
    double tds = 0.0; // Removed tds logic
    
    double rawTotal = taxableAmount + igst + cgst + sgst + shipping; // Removed tcs and tds
                      ...
                      GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => SalesInvoiceSelectedProductsScreen(
                                selectedProducts: widget.selectedProducts,
