...
class SalesInvoiceViewScreen extends StatefulWidget {
  final List<SalesInvoiceProduct> selectedProducts;
  ...
  @override
  State<SalesInvoiceViewScreen> createState() => _SalesInvoiceViewScreenState();
}

class _SalesInvoiceViewScreenState extends State<SalesInvoiceViewScreen> {
  ...
  void _sendEmail() async {
    ...
    debugPrint("--- Email Debug Info (Sales Invoice) ---");
      ...
      if (email.isEmpty || email == '0') {
        ...
        if (cusId != null) {
          ...
          try {
            ...
            debugPrint("Full Details Response (Sales Invoice): $res");
    ...
    final Uri emailLaunchUri = Uri(
      ...
      query: encodeQueryParameters(<String, String>{
        'subject': 'Invoice from Total ERP',
  ...
  Future<void> _convertTo(String targetType, String apiType) async {
    ...
    try {
      ...
      final Map<String, String> body = {
        ...
        'total_tcs': '0.00',
        'payment_mode': 'Cash',
        'products': json.encode(productsJsonList),
      };
    ...
    return PopScope(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(
              widget.invoiceMetadata?['invoice_no'] ?? 'INV - 1',
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Expanded(child: _customerField('GST Number', widget.selectedCustomer?['gst'] ?? 'N/A', sp, hp, vp)),
                                  ],
                                ),
                                SizedBox(height: 12 * vp),
                                _customerField('Phone Number', widget.selectedCustomer?['phone']?.toString() ?? widget.selectedCustomer?['Mobile']?.toString() ?? 'N/A', sp, hp, vp),
                                SizedBox(height: 12 * vp),
                                _customerField('Billing Address', widget.selectedCustomer?['address'] ?? 'N/A', sp, hp, vp),
                              ],
                            ),
                          ),
                      ],
      ...
      children: [
        if (_isFabExpanded) ...[
          ...
          Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              _dialItem(icon: Icons.share, label: 'Share', sp: sp, vp: vp, onTap: _shareInvoice),
              SizedBox(width: 10 * hp),
              _mainFabToggle(),
            ],
          ),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(2))),
              SizedBox(height: 20 * vp),
              _convertRow(Icons.sync_alt, 'Sales Invoice to Sales order', sp, hp, vp, onTap: () => _convertTo('Sales order', '8001')),
              _convertRow(Icons.sync_alt, 'Sales Invoice to Delivery Challan', sp, hp, vp, onTap: () => _convertTo('Delivery Challan', '8003')),
              _convertRow(Icons.sync_alt, 'Sales Invoice to Proforma Invoice', sp, hp, vp, onTap: () => _convertTo('Proforma Invoice', '8000')),
              SizedBox(height: 20 * vp),
            ],
          ),
        );
