...
class ProformaGenerateInfoScreen extends StatefulWidget {
  final String title;
  const ProformaGenerateInfoScreen({super.key, this.title = 'Invoice'});

  @override
  State<ProformaGenerateInfoScreen> createState() =>
      _ProformaGenerateInfoScreenState();
}

class _ProformaGenerateInfoScreenState
    extends State<ProformaGenerateInfoScreen> {
  ...
  Future<void> _fetchInvoiceNumber() async {
    try {
      ...
      // Try multiple possible keys for the invoice number
      ...
      if (isSuccess && generatedNumber != null) {
        setState(() {
          _invoiceNoController.text = generatedNumber.toString();
        });
      }
    } catch (e) {
  ...
  void _fetchAllDropdowns() {
    _fetchDropdownData(
        '17', _invoiceTypes, (val) => _selectedInvoiceType = val);
    _fetchDropdownData('16', _priceTypes, (val) => _selectedPriceType = val);
    _fetchDropdownData('26', _taxTypes, (val) => _selectedTaxType = val);
  }

  Future<void> _fetchDropdownData(String listId, List<String> targetList,
      Function(String) onSelectUpdate) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final cid = prefs.getString('cid') ?? '44555666';
      ...
      if (res['error'] == false && res['dropdown'] != null) {
        ...
        if (uniqueItems.isNotEmpty) {
          setState(() {
            ...
            // We need to re-fetch the current selected value because it might have changed
            // Actually, we pass in a setter.
            // But we only want to update if current selection is NOT in new list.
          });
        }
      }
    } catch (e) {
  ...
  Future<void> _fetchCustomers() async {
    ...
    try {
      ...
      debugPrint("Fetch Customers response: $jsonResponse");
      ...
      debugPrint("❌ ProformaGenerateInfoScreen => FETCH CUSTOMERS ERROR: $e");
    } finally {
  ...
  void _filterCustomers(String query) {
    setState(() {
      _filteredCustomers = _customers
          ...
          .toList();
      debugPrint("Filtered Customers: $_filteredCustomers");
    });
  }
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      setState(() {
                        ...
                        // Removed auto-show: _isCustomerDetailsVisible = true;
                      });
                    },
                  );
                },
                  ...
                  secondChild: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildReadOnlyField(
                          text:
                              _selectedCustomer?['phone']?.toString() ?? 'N/A',
                          screenWidth: screenWidth,
                          valueFontSize: valueFontSize,
                        ),
      ...
      child: ListTile(
        ...
        onTap: () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => ProformaCatalogScreen(
