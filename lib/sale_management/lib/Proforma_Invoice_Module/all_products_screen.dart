...
class ProformaCatalogScreen extends StatefulWidget {
  ...
  @override
  State<ProformaCatalogScreen> createState() => _ProformaCatalogScreenState();
}

class _ProformaCatalogScreenState extends State<ProformaCatalogScreen> {
  ...
  List<ProformaInvoiceProduct> _products = [];
  ...
  Future<void> _fetchProducts() async {
    ...
    try {
        ...
        setState(() {
          _products = data.map((item) {
            return ProformaInvoiceProduct(
              ...
              stock: int.tryParse(item['stock_qty'].toString()) ?? 0,
              ...
              taxRate:
                  double.tryParse(item['tax_rate']?.toString() ?? '0') ?? 0.0,
            );
          }).toList();
      ...
      debugPrint("❌ ProformaCatalogScreen => FETCH ERROR: $e");
    } finally {
      setState(() => _isLoading = false);
    }
  }

  List<ProformaInvoiceProduct> get _filteredProducts {
    List<ProformaInvoiceProduct> result = _products;
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => ProformaInvoicePaymentScreen(
