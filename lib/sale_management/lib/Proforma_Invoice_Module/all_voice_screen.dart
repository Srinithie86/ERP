...
List<ProformaInvoiceItem> _allInvoices = [];
bool _isLoading = false;

// ─── Main Page ────────────────────────────────────────────
class ProformaAllInvoicePage extends StatefulWidget {
  final String title;
  const ProformaAllInvoicePage({super.key, this.title = 'All Invoice'});

  @override
  State<ProformaAllInvoicePage> createState() => _ProformaAllInvoicePageState();
}

class _ProformaAllInvoicePageState extends State<ProformaAllInvoicePage> {
  ...
  Future<void> _fetchInvoices() async {
    ...
    try {
      ...
      debugPrint("Proforma Invoice Response: $res");
      if (res['error'] == false && res['invoices'] != null) {
        final List<dynamic> data = res['invoices'];
        setState(() {
      ...
      debugPrint("Error fetching proforma invoices (8008): $e");
    } finally {
      setState(() => _isLoading = false);
    }
  }

  List<ProformaInvoiceItem> get filteredInvoices {
    List<ProformaInvoiceItem> result = _allInvoices;
    if (_selectedFilterIndex == 1) {
      result = result.where((i) => i.status == 'APPROVED').toList();
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => ProformaGenerateInfoScreen(title: widget.title),
                    ),
                  );
                },
                child: Container(
  ...
  Widget _buildInvoiceCard(
    ...
    return GestureDetector(
      onTap: () {
        ...
        final List<ProformaInvoiceProduct> mappedProducts = productsRaw.map((p) {
          return ProformaInvoiceProduct(
            ...
            taxRate: double.tryParse(p['mtax']?.toString() ?? '0') ?? 0,
            discountAmount: double.tryParse(p['discount']?.toString() ?? '0') ?? 0,
            isPercentageDiscount: false,
          );
        }).toList();

        final summary = ProformaInvoiceSummary(
