...
class DirectInvoiceViewScreen extends StatefulWidget {
  ...
  @override
  State<DirectInvoiceViewScreen> createState() => _DirectInvoiceViewScreenState();
}

class _DirectInvoiceViewScreenState extends State<DirectInvoiceViewScreen> {
  ...
  void _sendEmail() async {
    ...
    debugPrint("--- Email Debug Info (Direct Invoice) ---");
      ...
      if (email.isEmpty || email == '0') {
        ...
        if (cusId != null) {
          ...
          try {
            ...
            debugPrint("Full Details Response (Direct Invoice): $res");
    ...
    final Uri emailLaunchUri = Uri(
      ...
      query: encodeQueryParameters(<String, String>{
        'subject': 'Direct Invoice from Total ERP',
                    ...
                    child: Column(
                      children: [
                        ListTile(
                          ...
                          title: Text(widget.selectedCustomer != null ? '${widget.selectedCustomer!['Ledger_Name']} - ${widget.selectedCustomer!['id']}' : 'No Customer', style: TextStyle(fontSize: 14 * sp, fontWeight: FontWeight.w700, fontFamily: 'Poppins')),
                          trailing: IconButton(
                          ...
                          Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                Row(
                                  children: [
                                    Expanded(child: _customerField('Contact Person', widget.selectedCustomer?['Ledger_Name']?.toString() ?? 'N/A', sp, hp, vp)),
                                    SizedBox(width: 12 * hp),
                                    Expanded(child: _customerField('GST Number', widget.selectedCustomer?['gst']?.toString() ?? 'N/A', sp, hp, vp)),
                                  ],
                                ),
                                SizedBox(height: 12 * vp),
                                _customerField('Phone Number', widget.selectedCustomer?['Mobile']?.toString() ?? 'N/A', sp, hp, vp),
                                SizedBox(height: 12 * vp),
                                _customerField('Billing Address', widget.selectedCustomer?['address']?.toString() ?? 'N/A', sp, hp, vp),
                              ],
                            ),
                          ),
                      ],
  ...
  Widget _dialItem({required IconData icon, required String label, required double sp, required double vp, String? customLabel, VoidCallback? onTap}) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(2))),
              SizedBox(height: 20 * vp),
              _convertRow(Icons.sync_alt, 'Direct Invoice to Sales Invoice', sp, hp, vp, onTap: () => _convertTo('Sales Invoice', '8005')),
              _convertRow(Icons.sync_alt, 'Direct Invoice to Delivery Challan', sp, hp, vp, onTap: () => _convertTo('Delivery Challan', '8003')),
              _convertRow(Icons.sync_alt, 'Direct Invoice to Proforma Invoice', sp, hp, vp, onTap: () => _convertTo('Proforma Invoice', '8000')),
              SizedBox(height: 20 * vp),
            ],
          ),
        );
