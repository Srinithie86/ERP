...
class SalesOrderInvoiceViewScreen extends StatefulWidget {
  ...
  const SalesOrderInvoiceViewScreen({
    super.key,
  ...
  @override
  State<SalesOrderInvoiceViewScreen> createState() =>
      _SalesOrderInvoiceViewScreenState();
}

class _SalesOrderInvoiceViewScreenState
    extends State<SalesOrderInvoiceViewScreen> {
  ...
  void _sendEmail() async {
    ...
    debugPrint("--- Email Debug Info (Sales Order) ---");
    ...
    if (customer != null) {
      ...
      if (email.isEmpty || email == '0') {
        ...
        if (cusId != null) {
          debugPrint(
              "Email missing. Fetching full details for Customer ID: $cusId");
          try {
            ...
            debugPrint("Full Details Response (Sales Order): $res");
            if (res['error'] == false &&
                res['data'] != null &&
              ...
              if (match != null) {
                ...
                debugPrint(
                    "Successfully matched Customer ID $cusId from API. Email: '$email'");
              }
            }
          } catch (e) {
    ...
    final Uri emailLaunchUri = Uri(
      ...
      query: encodeQueryParameters(<String, String>{
        'subject': 'Sales Order from Total ERP',
  ...
  Future<void> _convertTo(String targetType, String apiType) async {
    ...
    try {
      final meta = widget.invoiceMetadata;
      final prefs = await SharedPreferences.getInstance();
      
      final cid = meta?['cid'] ?? prefs.getString('cid') ?? '44555666';
      final uid = meta?['uid'] ?? prefs.getString('uid') ?? '1';
      ...
      // Calculation for summary totals matching logic in build()
      ...
      final double finalPayable = (taxableAmount + tax).roundToDouble();
      ...
      final Map<String, String> body = {
        ...
        'taxable_total': taxableAmount.toStringAsFixed(2),
        'cgst': (tax / 2).toStringAsFixed(2),
        'sgst': (tax / 2).toStringAsFixed(2),
        'igst': '0.00',
        'taxtotal': tax.toStringAsFixed(2),
        'total_gst': tax.toStringAsFixed(2),
        'g_total': finalPayable.toStringAsFixed(2),
        'grand_total': finalPayable.toStringAsFixed(2),
        'discount': '0.00',
        's_type': '1',
        'mtax': '2',
        'price_type': (widget.invoiceMetadata?['price_type']?.toString() == '2') ? '2' : '1',
        'tcs_type': '0',
        'total_tds': '0.00',
        'total_tcs': '0.00',
        'payment_mode': 'Cash',
      ...
      if (apiType == '8005') {
        ...
        final List<Map<String, dynamic>> productsJsonList =
          ...
          return {
            ...
            "total": totalItem,
            "cgst": (totalItem - taxableVal) / 2,
            "sgst": (totalItem - taxableVal) / 2,
            "igst": 0,
      ...
      if (res['error'] == false) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text('Converted to $targetType Successfully'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(res['error_msg'] ?? 'Conversion Failed'),
  ...
  Widget build(BuildContext context) {
    ...
    final double taxableAmount = totalSubtotal - totalDiscount;
    final double tax = taxableAmount * 0.18;
    const double shipping = 0.0; // Shipping removed as per UI
    final double rawTotal =
        taxableAmount + tax + shipping; // Removed tcs and tds
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        const Icon(Icons.person,
                            color: Color(0xFF005BBF), size: 20),
                        SizedBox(width: 8 * hp),
                    ...
                    Container(
                      decoration: BoxDecoration(
                        ...
                        border: const Border(
                            left:
                                BorderSide(color: Color(0xFF005BBF), width: 5)),
                        boxShadow: [
                          BoxShadow(
                      ...
                      child: Column(
                        children: [
                          ListTile(
                            ...
                            title: Text(
                                widget.selectedCustomer?['Ledger_Name'] ??
                                    'Common Customer',
                                style: TextStyle(
                                    fontSize: 14 * sp,
                                    fontWeight: FontWeight.w700,
                    ...
                    // ── ITEMS SECTION ──────────────────────────
                    Text('Item(${widget.selectedProducts.length})',
                        style: TextStyle(
                            fontSize: 15 * sp,
                            fontWeight: FontWeight.w700,
                            fontFamily: 'Poppins')),
                            ...
                            children: [
                              ListTile(
                                contentPadding: EdgeInsets.symmetric(
                                    horizontal: 16 * hp, vertical: 8 * vp),
                                title: Row(
                                  children: [
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                            ...
                                            children: [
                                              ...
                                              GestureDetector(
                                                ...
                                                child: Icon(
                                                  ...
                                                  size: 22 * sp,
                                                  color:
                                                      const Color(0xFF3D5481),
                                                ),
                                              ),
                                              const Spacer(),
                                              Text(
                                                  '₹${(p.price * p.selectedQty - (p.isPercentageDiscount ? (p.price * (p.discountPercentage / 100) * p.selectedQty) : (p.discountAmount * p.selectedQty))).toInt()}',
                                                  style: TextStyle(
                                                      color: const Color
                                                          .fromARGB(
                                                          255, 113, 191, 132),
                                          ...
                                          Text(
                                              'x ${p.selectedQty.toStringAsFixed(2)} OTH',
                                              style: TextStyle(
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Restored Old Row Layout for Summary with new fields
                          _summaryRow(
                              'Taxable Total',
                          ...
                          _summaryRow(
                              ...
                              Colors.black87),
                          SizedBox(height: 12 * vp),
                          _summaryRow('IGST', '₹ 0.00', sp, Colors.black87),
                          SizedBox(height: 12 * vp),
      ...
      children: [
        if (_isFabExpanded) ...[
          Row(
            ...
            children: [
              _dialItem(
                  icon: Icons.currency_rupee, label: 'Record', sp: sp, vp: vp),
  ...
  Widget _mainFabToggle() {
    return FloatingActionButton(
      ...
      child: Icon(_isFabExpanded ? Icons.close : Icons.add,
          color: Colors.white, size: _isFabExpanded ? 28 : 30),
    );
  }
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  _mediaOption(
                      Icons.upload_outlined, 'Upload File', sp, hp, vp),
                ],
              ),
            ],
          ),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 20 * vp),
              _convertRow(
                  Icons.sync_alt, 'Sales Order to Sales Invoice', sp, hp, vp,
                  onTap: () => _convertTo('Sales Invoice', '8005')),
              _convertRow(
                  Icons.sync_alt, 'Sales Order to Proforma Invoice', sp, hp, vp,
                  onTap: () => _convertTo('Proforma Invoice', '8000')),
              _convertRow(
                  Icons.sync_alt, 'Sales Order to Delivery Challan', sp, hp, vp,
                  onTap: () => _convertTo('Delivery Challan', '8003')),
              SizedBox(height: 20 * vp),
            ],
          ),
        );
  ...
  Widget _mediaOption(
      IconData icon, String label, double sp, double hp, double vp) {
    return Column(
      children: [
        Container(
          width: 70 * sp,
              ...
              Wrap(
                ...
                children: [
                  ...
                  _OptionItem(
                      ...
                      label: 'Convert E-Way Bill',
                      iconColor: const Color(0xFF9CCC65),
                      sp: sp,
                      vp: vp),
                  _OptionItem(
                      icon: Icons.computer,
                      label: 'Convert E-Invoice',
  ...
  Widget _invoiceItemDetail(
      IconData icon, String label, String value, double sp, double hp) {
    return Row(children: [
      Icon(icon, color: const Color(0xFF005BBF), size: 18 * sp),
      SizedBox(width: 8 * hp),
      Text(label,
          style: TextStyle(
              color: const Color(0xFF005BBF),
  ...
  Widget _summaryRow(String label, String value, double sp, Color valueColor,
      {bool hasInfo = false}) {
  ...
  void _shareInvoice() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Sharing initialized...')),
    );
  }
  ...
  Future<void> _launchPDF() async {
    ...
    debugPrint("Opening PDF in-app: ${widget.pdfUrl}");
    if (mounted) {
      Navigator.push(
        context,
        MaterialPageRoute(
        ...
        child: Column(
          children: [
            Container(
                ...
                decoration: BoxDecoration(
                    color: isCancel
                        ? const Color(0xFFEF9A9A).withOpacity(0.3)
                        : const Color(0xFFE0F7FA),
                    borderRadius: BorderRadius.circular(12)),
                child: Icon(icon,
                    color: isCancel ? Colors.red : iconColor, size: 22 * sp)),
            SizedBox(height: 6 * vp),
            Text(label,
                textAlign: TextAlign.center,
                style: TextStyle(
